...
import com.cj.jplugin.utils.JNIUtils;
import com.cj.jplugin.utils.ReflectAccelerator;
...
public class BundleParser {
    ...
    public BundleParser(Context context,File sourceFile, String packageName) {
        mArchiveSourcePath = sourceFile.getPath();
        mPackageName = packageName;
        mContext = context;
    }

    public static BundleParser parsePackage(Context context,File sourceFile, String packageName) {
        if (sourceFile == null || !sourceFile.exists()) return null;

        BundleParser bp = new BundleParser(context,sourceFile, packageName);
    ...
    private boolean parsePackage(Resources res, XmlResourceParser parser) {
        ...
        try {
            ...
            if (abiFlags != 0) {
                String abi = JNIUtils.getExtractABI(abiFlags, PluginManager.is64bit());
                if (abi != null) {
                    mLibDir = "lib/" + abi + "/";
                }
            }
